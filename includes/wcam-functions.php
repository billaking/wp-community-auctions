 ...
 */
function wcam_format_price($amount) {
    $symbol = get_option('wcam_currency_symbol', '$');
    $position = get_option('wcam_currency_position', 'before');
 ...
 */
function wcam_get_current_bid($auction_id) {
    $current_bid = get_post_meta($auction_id, '_wcam_current_bid', true);

    if (empty($current_bid)) {
        $current_bid = get_post_meta($auction_id, '_wcam_starting_price', true);
    }
 ...
 */
function wcam_get_bid_count($auction_id) {
    global $wpdb;
 ...
 */
function wcam_get_highest_bidder($auction_id) {
    $user_id = get_post_meta($auction_id, '_wcam_highest_bidder', true);
 ...
 */
function wcam_is_user_winning($auction_id, $user_id = 0) {
    if (!$user_id) {
 ...
 */
function wcam_has_user_bid($auction_id, $user_id = 0) {
    if (!$user_id) {
        $user_id = get_current_user_id();
    }
 ...
 */
function wcam_get_time_remaining($auction_id) {
    ...
    if ($diff <= 0) {
        return array('ended' => true, 'time' => __('Auction Ended', 'wp-community-auction-manager'));
    }

    return array(
        'ended' => false,
        'time' => wcam_format_time_remaining($diff),
        'seconds' => $diff,
    );
}
 ...
 */
function wcam_format_time_remaining($seconds) {
    ...
    if ($days > 0) {
        $parts[] = sprintf(_n('%d day', '%d days', $days, 'wp-community-auction-manager'), $days);
    }

    if ($hours > 0 || $days > 0) {
        $parts[] = sprintf(_n('%d hour', '%d hours', $hours, 'wp-community-auction-manager'), $hours);
    }

    if ($days == 0) {
        $parts[] = sprintf(_n('%d minute', '%d minutes', $minutes, 'wp-community-auction-manager'), $minutes);
    }

    if ($days == 0 && $hours == 0) {
        $parts[] = sprintf(_n('%d second', '%d seconds', $secs, 'wp-community-auction-manager'), $secs);
    }
 ...
 */
function wcam_is_auction_active($auction_id) {
 ...
 */
function wcam_get_minimum_bid($auction_id) {
 ...
 */
function wcam_get_status_label($auction_id) {
    $status = get_post_meta($auction_id, '_wcam_auction_status', true);
